...
class SelectionSorterTest {
    ...
    private SortingProblem selectionSorter = new SelectionSorter();

    @Test
    void selection_sort_with_empty_array() {
        assertEquals(0, selectionSorter.sort(new double[0]).length);
    }
    ...
    void bubble_sort_with_already_sorted_array() {
        double[] unsorted = { 1, 2 };
        double[] expected = { 1, 2 };
        assertArrayEquals(expected, selectionSorter.sort(unsorted));
    }
    @Test
    ...
    @Test
    void selection_sort_with_decimal_point() {
